...
for i in range(16):
    ...
    print(pred_shape)
    axis.axvline(
        gt_shape[0], linewidth=2, label="Ground Truth Shape Params 0", color="red"
    )
    axis.axvline(
        gt_shape[1], linewidth=2, label="Ground Truth Shape Params 1", color="blue"
    )
    axis.set_xlim(0, 4)
    colors = ["red", "blue"]
    for j in range(2):
